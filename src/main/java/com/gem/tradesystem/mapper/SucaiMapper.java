...
public interface SucaiMapper {
    ...
    @Select("SELECT * from sucai where status=3 ORDER BY downnum DESC LIMIT 0,3")//获取最畅销的素材，前三
    List<Sucai> getTopSale();

    @Select("SELECT * from sucai where status=3 ORDER BY favnum DESC LIMIT 0,3")//获取最畅销的素材，前三
